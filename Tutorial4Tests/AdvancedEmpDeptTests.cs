...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = (from e1 in emps join e2 in emps on e1.Mgr equals e2.EmpNo select new {  Employee = e1.EName,
            Manager = e2.EName }).ToList();
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = (from e in emps let total = e.Sal + (e.Comm ?? 0) select new { EName = e.EName, Total = total })
            .ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = (from e in emps join d in depts on e.DeptNo equals d.DeptNo from s in grades where e.Sal >= s.Losal && e.Sal <= s.Hisal select new{EName = e.EName, DName = d.DName,Grade = s.Grade}).ToList();
